...
export default function Playback({ src, generated, onTimeMs }: { src: string; generated: boolean; onTimeMs?: (ms: number) => void }) {
  ...
  return <section className="playback" aria-label="Playback">
    ...
    <div className="transport"><button className="primary play" aria-label={playing ? 'Pause' : 'Play'} onClick={toggle}>{playing ? 'Ⅱ' : '▶'}</button><button onClick={() => { if (audio.current) { audio.current.currentTime = 0; setPosition(0) } }} aria-label="Restart">↺</button><label className="seek"><span className="sr-only">Playback position</span><input className="seek-input" type="range" min="0" max={duration || 1} step="0.1" value={position} disabled={!duration} onChange={event => { const time = Number(event.target.value); if (audio.current) audio.current.currentTime = time; setPosition(time) }} /></label><output className="time" aria-label="Current position">{clock(position)} / {clock(duration)}</output><label className="volume">Volume<input className="seek-input" type="range" min="0" max="1" step="0.01" value={volume} onChange={event => { const next = Number(event.target.value); setVolume(next); if (audio.current) audio.current.volume = next }} /></label></div>
