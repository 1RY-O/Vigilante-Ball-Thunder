...
export default function Playback({ src, generated }: { src: string; generated: boolean }) {
  ...
  return <section className="playback" aria-label="Playback">
    <audio ref={audio} src={src} preload="metadata" onLoadedMetadata={() => setDuration(Number.isFinite(audio.current!.duration) ? audio.current!.duration : 0)} onTimeUpdate={() => setPosition(audio.current!.currentTime)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)} onError={() => { setPlaying(false); setError('This audio cannot be played in your browser. Your score and exports are still available.') }} />
    <div className="playback-heading"><strong>{generated ? 'Transcribed playback' : 'Original recording'}</strong><span>No note-following data available</span></div>
