...
import { formatBytes, formatDurationSec, formatTempoBpm } from './format'
import './App.css'
...
export default function App() {
  ...
  // Quality-strip values. The completed result carries the backend's own
  // provenance and measurements; capabilities fill the engine label only while
  // the result has none. A field nobody returned is omitted, never invented.
  // Server-measured values win over the client's own probe/stopwatch.
  const metadataEngine = result?.engineUsed ?? (caps?.engine ? (caps.engine.mock ? `${caps.engine.name ?? 'stub'} (MOCK)` : caps.engine.name ?? null) : null)
  const metadataModel = caps?.engine?.model
  const metadataDuration = result?.durationSec ?? probedDuration
  const metadataTranscriptionMs = result?.transcriptionMs ?? transcriptionMs
  const metadataInstruments = result?.detectedInstruments?.length ? result.detectedInstruments : null
  const metadataTempo = result?.metadata?.tempoBpm
  const metadataKey = result?.metadata?.keyName
  useEffect(() => {
    ...
    <main>
      ...
      <div className="workspace">
        <section className="upload-section" aria-labelledby="upload-title"><div className="section-heading"><span className="section-number">01</span><h2 id="upload-title">Start with a recording</h2></div>
          ...
          {busy && <progress className="job-progress" aria-label={labels[stage]} max={100} value={progress} />}
